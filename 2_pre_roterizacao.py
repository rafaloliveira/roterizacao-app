...
def pagina_confirmar_producao():
    ...
    with col_total_4:
      ...
      # --- NOVO: Cálculo e exibição do badge para 'AGENDAR' sem 'Entrega Programada' ---
    entregas_agendar_sem_programacao = df[
        (df['Status'].astype(str).str.upper() == 'AGENDAR') & # Garante que 'Status' é tratado como string
        (df['Entrega Programada'].isnull() | (df['Entrega Programada'] == '')) # Verifica se é nulo ou string vazia
    ]
    ...
    with col_badge_agendar:
        st.markdown(badge(
            ...
            background_color="#FF4500", # Laranja avermelhado para destaque
            text_color="white"
        ), unsafe_allow_html=True)

    # --- NOVO: Cálculo e exibição do badge para 'Particularidade' preenchida ---
    entregas_com_particularidade = df[
        (df['Particularidade'].astype(str).str.strip() != '') & # Garante que 'Particularidade' é string e não está vazia
        (df['Particularidade'].notna()) # Garante que não é NaN
    ]
    ...
    with col_badge_particularidade:
        st.markdown(badge(
            ...
            background_color="#DAA520", # Amarelo ouro para destaque
            text_color="white"
        ), unsafe_allow_html=True)
...
""")
    ...
    for cliente_pagador in clientes_pagadores_unicos:
        ...
        <div style="margin-top:20px;padding:10px;background:#333333;border-left:4px solid #6c757d;border-radius:6px;display:inline-block;max-width:100%;">
            <strong style="color:white;">Cliente Pagador:</strong> <span style="color:white;">{cliente_pagador}</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_badge:
            ...
            volumes_sum = df_cliente['Quantidade de Volumes'].sum() if 'Quantidade de Volumes' in df_cliente.columns else 0
             # --- Cálculo para os badges específicos do cliente (já existe) ---
...
""")
    ...
    for rota_visual in sorted(df_visivel["Rota_Grupo"].dropna().unique()):
        ...
        # Converte o array NumPy para uma lista Python e garante que os elementos sejam strings
        # O 'if pd.notna(r)' garante que qualquer NaN remanescente seja filtrado
        regioes_limpas = [str(r).strip() for r in regioes if pd.notna(r)] 
        
        if len(regioes_limpas) > 1:
            regiao_display = " / ".join(regioes_limpas)
        elif len(regioes_limpas) == 1:
            regiao_display = regioes_limpas[0]
        else: # Este bloco será executado se len(regioes_limpas) == 0
            regiao_display = "–"
        ...
        entregas_agendar_sem_programacao_rota = df_rota[
            (df_rota['Status'].astype(str).str.upper() == 'AGENDAR') &
            (df_rota['Entrega Programada'].isnull() | (df_rota['Entrega Programada'] == ''))
        ]
        qtd_agendar_sem_programacao_rota = len(entregas_agendar_sem_programacao_rota)

        entregas_com_particularidade_rota = df_rota[
            (df_rota['Particularidade'].astype(str).str.strip() != '') &
            (df_rota['Particularidade'].notna())
        ]
        qtd_com_particularidade_rota = len(entregas_com_particularidade_rota)


        st.markdown(
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>{len(df_rota)} entregas</span>" +
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>{formatar_brasileiro(df_rota['Peso Calculado em Kg'].sum())} kg calc</span>" +
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>{formatar_brasileiro(df_rota['Peso Real em Kg'].sum())} kg real</span>" +
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>Valor frete: R$ {formatar_brasileiro(df_rota['Valor do Frete'].sum())}</span>" +
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>{formatar_brasileiro(df_rota['Cubagem em m³'].sum())} m³</span>" +
            f"<span style='background:#333333;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>{int(df_rota['Quantidade de Volumes'].sum())} volumes</span>" +
            f"<span style='background:#e06200;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>📅  Agenda s/ Prog.: {qtd_agendar_sem_programacao_rota}</span>" +
            f"<span style='background:#b8860b;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>⚠️ Com Particularidade: {qtd_com_particularidade_rota}</span>",
            unsafe_allow_html=True
        )
                ...
                if not selecionadas.empty:
                    ...
                    with col_aprov:
                        ...
                        if st.button(
                            ...
                            disabled=(selecionadas.empty) or (valor_contratacao <= 0) # <--- AQUI ESTÁ A CORREÇÃO
                        ):
        ...
        with st.expander("🔽 Ver entregas da carga fechada", expanded=False):
            with st.spinner("   Formatando entregas da carga fechada..."):
                ...
                date_only_formatter = JsCode("""
                    function(params) {
                        if (params.value === null || typeof params.value === 'undefined' || params.value === '') return '';
                        const parts = params.value.split(' ')[0]; // Pega só a parte da data
                        return parts;
                    }
                """)
                # Formata o DataFrame para exibição no AgGrid
                df_formatado = df_carga[[col for col in colunas_exibir if col in df_carga.columns]].copy()
                df_formatado = apply_brazilian_date_format_for_display(df_formatado)
                ...
                if df_formatado.empty or df_formatado.shape[1] == 0:
                    ...
                    if 'data_fechamento' in df_formatado.columns:
                        gb.configure_column('data_fechamento', valueFormatter=JsCode("""
                            function(params) {
                                if (params.value) {
                                    return params.value; // A string já virá formatada como dd/mm/yyyy hh:mm
                                }
                                return '';
                            }
                        """))
                    ...
                    # Formatação específica para as novas colunas de auditoria que são timestamps completos
                    for col in ['data_aprovacao_custos', 'pdf_downloaded_at', 'csv_downloaded_at', 'Data_Hora_Gerada']:
                        if col in df_formatado.columns:
                            gb.configure_column(col, valueFormatter=JsCode("""
                                function(params) {
                                    if (params.value) {
                                        return params.value; // Já está em dd-mm-yyyy HH:MM
                                    }
                                    return '';
                                }
                            """))
