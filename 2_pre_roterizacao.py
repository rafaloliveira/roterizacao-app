...
# ==============================================================================
# NOVAS CONSTANTES PARA SINCRONIZAÇÃO DETALHADA
# ==============================================================================

# Defina esta constante se ainda não estiver global:
...
FLOW_TABLES = [
    ...
    "aprovacao_custos",
    "cargas_aprovadas"
    # 'cargas_fechadas' está intencionalmente fora, conforme sua solicitação
]

# Lista exata de colunas que devem ser sincronizadas entre fBaseroter e as tabelas do fluxo.
# Apenas estas colunas terão suas informações atualizadas em todas as etapas do fluxo.
SYNCABLE_COLUMNS = [
    "Serie_Numero_CTRC",
    "Cliente Pagador",
    "Chave CT-e",
...
def sincronizar_fluxo_com_fbaseroter(supabase_client):
    ...
    sync_log_list = [] # Lista para coletar os logs de operações
    ...
    try:
        ...
        if df_fbaseroter_active.empty:
            ...
            for table_name in FLOW_TABLES:
                ...
                response_all_current = supabase_client.table(table_name).select("*").execute()
                df_all_current = pd.DataFrame(response_all_current.data)
                if not df_all_current.empty:
                    deleted_ctrcs_for_log = df_all_current['Serie_Numero_CTRC'].tolist()
                    supabase_client.table(table_name).delete().neq("Serie_Numero_CTRC", "").execute()
                    for ctrc in deleted_ctrcs_for_log:
                        sync_log_list.append({
                            'change_type': 'deleted',
                            'table_name': table_name,
                            'ctrc': ctrc,
                            'details_json': {'reason': 'Registro removido da fBaseroter (fBaseroter vazia).'}
                        })
        ...
        df_fbaseroter_active.set_index('Serie_Numero_CTRC', inplace=True) # Para lookup rápido
        ...
        for table_name in FLOW_TABLES:
            ...
            try:
                ...
                # DEBUG: Inspeciona as colunas retornadas para cada tabela de fluxo
                st.write(f"DEBUG: Colunas em df_flow_table para {table_name}: {df_flow_table.columns.tolist()}")
                st.write(f"DEBUG: 'Serie_Numero_CTRC' está nas colunas? {'Serie_Numero_CTRC' in df_flow_table.columns}")


                # Garante que 'Serie_Numero_CTRC' seja string e não tenha nulos para ser a chave principal
                if 'Serie_Numero_CTRC' not in df_flow_table.columns:
                    st.warning(f"Coluna 'Serie_Numero_CTRC' não encontrada em '{table_name}'. Pulando esta tabela para sincronização.")
                    continue
                ...
                df_flow_table.set_index('Serie_Numero_CTRC', inplace=True) # Para lookup rápido

                current_flow_ctrcs = set(df_flow_table.index.tolist())
                active_fbaseroter_ctrcs = set(df_fbaseroter_active.index.tolist())
                ...
                if ctrcs_to_delete_from_flow_table:
                    ...
                    # Coleta dados antes da deleção para log
                    deleted_records_df = df_flow_table.loc[list(ctrcs_to_delete_from_flow_table), :].copy()
                    
                    supabase_client.table(table_name).delete().in_("Serie_Numero_CTRC", list(ctrcs_to_delete_from_flow_table)).execute()
                    
                    for ctrc, row in deleted_records_df.iterrows():
                        sync_log_list.append({
                            'change_type': 'deleted',
                            'table_name': table_name,
                            'ctrc': ctrc,
                            'details_json': {'reason': 'Registro não mais presente na fBaseroter ativa.', 'data_before_deletion': row.to_dict()}
                        })
                else:
                    st.info(f"Nenhum registro a ser deletado de '{table_name}'.")

                # ==================================
                # Operações de ATUALIZAÇÃO / INSERÇÃO (UPSERT)
                # ==================================
                # Entregas que estão na fBaseroter ativa e que DEVEM estar nesta tabela (seja por update ou nova inserção)
                ctrcs_for_upsert = active_fbaseroter_ctrcs.intersection(current_flow_ctrcs) # Atualizações
                ctrcs_for_upsert.update(active_fbaseroter_ctrcs - current_flow_ctrcs) # Novas inserções (se a lógica de entrada já não tratou)

                if ctrcs_for_upsert:
                    df_to_upsert = df_fbaseroter_active.loc[list(ctrcs_for_upsert), :].copy()

                    # Prepara para UPSERT (formata datas, nulos, etc.)
                    df_to_upsert_prepared = _prepare_df_for_supabase_insert(df_to_upsert.reset_index()) # reset_index para 'Serie_Numero_CTRC' virar coluna de novo

                    records_to_upsert = df_to_upsert_prepared.to_dict(orient='records')

                    if records_to_upsert:
                        st.info(f"Realizando UPSERT de {len(records_to_upsert)} entrega(s) em '{table_name}'...")
                        
                        # --- Coletar diferenças para o log ANTES do UPSERT ---
                        updated_count = 0
                        for new_record in records_to_upsert:
                            ctrc = new_record['Serie_Numero_CTRC']
                            
                            # Tenta encontrar o registro antigo se ele já existia na tabela de fluxo
                            old_record = None
                            if ctrc in df_flow_table.index:
                                old_record_df = df_flow_table.loc[[ctrc], :].copy()
                                old_record_df_prepared = _prepare_df_for_supabase_insert(old_record_df.reset_index())
                                old_record = old_record_df_prepared.iloc[0].to_dict()

                            # Compara e registra as mudanças
                            if old_record:
                                changed_fields = {}
                                for col in SYNCABLE_COLUMNS:
                                    # É crucial que ambos old_record e new_record tenham os valores padronizados
                                    # (ex: NaT para None, Timestamps para ISO strings) para comparação precisa.
                                    # _prepare_df_for_supabase_insert faz isso.
                                    old_val = old_record.get(col)
                                    new_val = new_record.get(col)

                                    if str(old_val) != str(new_val): # Compara como string para lidar com tipos mistos
                                ...
                                if changed_fields:
                                    ...
                                    sync_log_list.append({
                                        ...
                                        'table_name': table_name,
                                        'ctrc': ctrc,
                                        'details_json': {'changed_fields': changed_fields}
                                    })
                        
                        # Realiza o UPSERT no Supabase
                        supabase_client.table(table_name).upsert(records_to_upsert).execute()
                        st.success(f"✅ {updated_count} entrega(s) atualizada(s) em '{table_name}'.")
                    else:
                        st.info(f"Nenhuma entrega em '{table_name}' para UPSERT (após preparação).")
                else:
                    st.info(f"Nenhuma entrega em '{table_name}' para UPSERT.")

                # Invalida o cache da tabela específica para forçar o recarregamento
                if table_name == "confirmadas_producao": st.session_state.pop("df_confirmadas_cache", None); st.session_state["reload_confirmadas_producao"] = True
                if table_name == "aprovacao_diretoria": st.session_state.pop("df_aprovacao_diretoria_cache", None); st.session_state["reload_aprovacao_diretoria"] = True
                if table_name == "pre_roterizacao": st.session_state.pop("df_pre_roterizacao_cache", None); st.session_state["reload_pre_roterizacao"] = True
                if table_name == "cargas_geradas": st.session_state.pop("df_cargas_cache", None); st.session_state["reload_cargas_geradas"] = True
                if table_name == "aprovacao_custos": st.session_state.pop("df_aprovacao_custos_cache", None); st.session_state["reload_aprovacao_custos"] = True
                if table_name == "cargas_aprovadas": st.session_state.pop("df_cargas_aprovadas_cache", None); st.session_state["reload_cargas_aprovadas"] = True
    ...
    st.success("✅ Sincronização do fluxo de trabalho concluída.")
    return sync_log_list # Retorna a lista de logs coletados
...
def salvar_totais_sincronizacao(qtd_confirmadas, qtd_pre_roterizacao, qtd_paletizadas, novas_confirmadas, novas_pre_roterizacao):
    try:
        totais_info = {
            "qtd_confirmadas": qtd_confirmadas,
            "qtd_pre_roterizacao": qtd_pre_roterizacao,
            "qtd_paletizadas": qtd_paletizadas,
            "novas_confirmadas": novas_confirmadas,
            "novas_pre_roterizacao": novas_pre_roterizacao
        }
        supabase.table("metadados").upsert({"chave": "totais_sincronizacao_atual", "valor": json.dumps(totais_info)}).execute()
    except Exception as e:
        st.warning(f"Erro ao salvar totais de sincronização em metadados: {e}")

def carregar_totais_sincronizacao():
    try:
        dados = supabase.table("metadados").select("valor").eq("chave", "totais_sincronizacao_atual").execute()
        if dados.data:
            valor_salvo = dados.data[0]["valor"]
        ...
        st.warning(f"Erro ao carregar totais de sincronização de metadados: {e}")
        return None

# Ajuste sua função salvar_historico_sincronizacao para chamar salvar_totais_sincronizacao
# Exemplo (se já tiver uma função salvar_historico_sincronizacao):
# def salvar_historico_sincronizacao(username, num_new_fbaseroter, num_confirmadas, num_pre_roterizacao, num_paletizadas, num_novas_confirmadas, num_novas_pre_roterizacao, file_name=None, file_hash=None):
#     # ... (seu código existente para historico_sincronizacoes) ...
#     salvar_totais_sincronizacao(num_confirmadas, num_pre_roterizacao, num_paletizadas, num_novas_confirmadas, num_novas_pre_roterizacao)

# Certifique-se de que o st.session_state.file_uploader_key seja inicializado
if 'file_uploader_key' not in st.session_state:
    st.session_state.file_uploader_key = 0

# Variável de estado para o ID da última operação de sincronização detalhada
if 'last_sync_operation_id' not in st.session_state:
    st.session_state.last_sync_operation_id = None

# Variável de estado para a mensagem de sucesso da sincronização (balões)
if "_sync_success_message_displayed" not in st.session_state:
    st.session_state._sync_success_message_displayed = False
...
def pagina_sincronizacao():
    ...
    # NOVO: Carregar os totais persistidos do Supabase
    # Este bloco substitui o if "_qtd_confirmadas" not in st.session_state:
    persistidos = carregar_totais_sincronizacao()

    if persistidos:
        st.session_state._qtd_confirmadas = persistidos.get("qtd_confirmadas", 0)
        st.session_state._qtd_pre_roterizacao = persistidos.get("qtd_pre_roterizacao", 0)
        st.session_state._qtd_paletizadas = persistidos.get("qtd_paletizadas", 0)
        st.session_state._novas_confirmadas = persistidos.get("novas_confirmadas", 0)
        st.session_state._novas_pre_roterizacao = persistidos.get("novas_pre_roterizacao", 0)
    else:
        # Se não houver dados persistidos, inicializa com 0 (primeira vez ou erro)
    ...
    # NOVO: Botão para limpar o histórico de logs de sincronização detalhado
    if st.button("🗑️ Limpar Histórico de Logs de Sincronização Detalhado"):
        try:
            supabase.table("sync_operation_log").delete().neq("id", "").execute()
            st.session_state.last_sync_operation_id = None # Limpa o ID da última operação
            st.success("Histórico de logs de sincronização detalhado limpo com sucesso!")
            st.rerun()
        except Exception as e:
            st.error(f"Erro ao limpar o histórico de logs: {e}")
    ...
    if st.session_state.sync_triggered:
        ...
        try:
            ...
            # --- NOVO: Chamar a função para sincronizar o fluxo de trabalho ---
            # Aqui, capturamos a lista de logs retornada pela função
            detailed_sync_logs = sincronizar_fluxo_com_fbaseroter(supabase)
            progress_bar.progress(60)

            # --- NOVO: Salvar os logs detalhados na nova tabela sync_operation_log ---
            if detailed_sync_logs:
                st.info(f"Salvando {len(detailed_sync_logs)} registros de log de sincronização...")
                ...
                supabase.table("sync_operation_log").insert(detailed_sync_logs).execute()
                st.session_state.last_sync_operation_id = sync_operation_uuid # Armazena o ID da última sync
                st.success(f"✅ {len(detailed_sync_logs)} eventos de sincronização registrados.")
            else:
                st.info("Nenhum evento de sincronização detalhado para registrar.")
            ...
            st.session_state.pop("df_aprovacao_diretoria_cache", None) # Adicionei essa linha, se faltou
            st.session_state["reload_pre_roterizacao"] = True
            st.session_state.pop("df_pre_roterizacao_cache", None)
            st.session_state.pop("dados_confirmados_cache", None) # Se essa variável for usada para cache
            # st.session_state["reload_rotas_confirmadas"] = True # Não encontrei essa flag, se não usa pode remover
            # st.session_state.pop("df_rotas_confirmadas_cache", None) # Não encontrei essa cache
    ...
    # --- NOVO: Exibição do Log de Sincronização Recente ---
    st.markdown("### Logs da Última Sincronização Detalhada")

    if st.session_state.last_sync_operation_id:
        try:
            with st.spinner("Carregando logs da última sincronização..."):
                response_logs = supabase.table("sync_operation_log")\
                                    .select("sync_timestamp, change_type, table_name, ctrc, details_json")\
                                    .eq("sync_operation_id", st.session_state.last_sync_operation_id)\
                                    .order("sync_timestamp", desc=True)\
                                    .execute()
                df_logs = pd.DataFrame(response_logs.data)

            if not df_logs.empty:
                # Formatar timestamp para exibição
                # Assegura que a coluna 'sync_timestamp' seja do tipo datetime antes de formatar
                df_logs['sync_timestamp'] = pd.to_datetime(df_logs['sync_timestamp'])
                df_logs['sync_timestamp'] = df_logs['sync_timestamp'].apply(formatar_data_hora_br)

                log_data_for_grid = []
                for idx, row in df_logs.iterrows():
                    entry = {
                        'Horário': row['sync_timestamp'],
                        'Tipo de Mudança': row['change_type'],
                    ...
                    if row['change_type'] == 'updated':
                        ...
                        for field, vals in changed_fields.items():
                            log_data_for_grid.append({
                                **entry,
                                'Campo Alterado': field,
                                'Valor Antigo': vals.get('old'),
                                'Valor Novo': vals.get('new'),
                                'Motivo/Detalhes': 'Atualização de campo'
                            })
                        ...
                        log_data_for_grid.append({
                            **entry,
                            ...
                            'Valor Antigo': 'N/A',
                            'Valor Novo': 'N/A',
                            'Motivo/Detalhes': details.get('reason', 'Registro deletado')
                        })
                
                if log_data_for_grid:
                    df_display_logs = pd.DataFrame(log_data_for_grid)
                    
                    gb = GridOptionsBuilder.from_dataframe(df_display_logs)
                    gb.configure_default_column(resizable=True, sortable=True, filter=True, minWidth=100)
                    gb.configure_column("Horário", minWidth=160)
                    gb.configure_column("Tipo de Mudança", minWidth=120)
                    gb.configure_column("Tabela", minWidth=120)
                    gb.configure_column("CTRC", minWidth=180)
                    gb.configure_column("Campo Alterado", minWidth=150)
                    gb.configure_column("Valor Antigo", minWidth=200)
                    gb.configure_column("Valor Novo", minWidth=200)
                    gb.configure_column("Motivo/Detalhes", minWidth=250)
                    ...
                    st.info("Nenhum log detalhado encontrado para a última sincronização.")
            else:
                st.info("Nenhum log detalhado encontrado para a última sincronização.")

        except Exception as e:
            st.error(f"Erro ao carregar os logs da última sincronização: {e}")
            st.exception(e)
    else:
        st.info("Nenhuma sincronização foi realizada ainda nesta sessão ou o histórico foi limpo.")
    ...
    for col in all_db_date_or_timestamp_cols:
            ...
            df_copy[col] = df_copy[col].apply(
                lambda x: (
                    ...
                    (x.tz_localize(timezone.utc, ambiguous='NaT', nonexistent='NaT').astimezone(FUSO_BRASIL)
                     ...
                     else (x.astimezone(FUSO_BRASIL) if pd.notna(x) and x.tzinfo is not None else pd.NaT)) # Se tz-aware, apenas converte
                )
            )
            format_str = DATE_DISPLAY_FORMAT_STRING # Para timestamps, usa HH:MM
        ...
        if format_str: # Aplica apenas se um formato foi determinado
            df_copy[col] = df_copy[col].apply(
                ...
                else '' # Trata NaT e valores não-datetime
            )
    return df_copy
#--------------------------------------------------------------------------------------------------------------------
