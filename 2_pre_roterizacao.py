            ...
            with st.expander("🔽 Ver entregas da carga", expanded=True):
                ...
                with col_aprov:
                    ...
                    save_button_disabled = (
                        ...
                        valor_contratacao_ui_normalized == valor_contratacao_db_normalized
                    )
...
def pagina_cargas_aprovadas():
    ...
    try:
        ...
        "Rota", "Regiao",  "Chave CT-e",
        "Particularidade", "Codigo da Ultima Ocorrencia", "Cubagem em m³", "Quantidade de Volumes",
        "valor_contratacao", "numero_carga", "motorista", "placa","Tipo_Tratativa_Especial",
...
def pagina_cargas_fechadas():
    ...
    for carga in cargas_unicas_paginated:
        ...
        valor_contratacao_carga_bruto = df_carga["valor_contratacao"].iloc[0] if "valor_contratacao" in df_carga.columns and not df_carga.empty else 0.0
        total_frete_carga_bruto = df_carga["Valor do Frete"].sum() if "Valor do Frete" in df_carga.columns and not df_carga.empty else 0.0
        
        # Converte para numérico e substitui NaN/Infinitos por 0.0 para garantir que sejam valores "limpos"
        ...
        # Agora o bloco de cálculo DECIMAL, usando os valores limpos:
        getcontext().prec = 28 # Mantém a alta precisão para cálculos intermediários

        # Converte para Decimal de forma segura (neste ponto, valor_contratacao_limpo e total_frete_limpo já são floats finitos)
        try:
            d_valor_contratacao_carga = Decimal(str(valor_contratacao_limpo))
            d_total_frete_carga = Decimal(str(total_frete_limpo))
        except InvalidOperation:
            # Esta exceção só deve ocorrer se 'str(valor)' ainda produzir algo inválido para Decimal,
            # o que é muito improvável após np.nan_to_num. Mantido como última salvaguarda.
            d_valor_contratacao_carga = Decimal('0')
            d_total_frete_carga = Decimal('0')
            st.warning(f"Carga {carga}: Aviso! Conversão final de valores para Decimal falhou de forma inesperada. Tratando como zero para cálculos.")
        ...
        # Verifica se d_total_frete_carga é zero APÓS a conversão para Decimal
        # (A checagem .is_finite() e .is_nan() teoricamente não seria mais necessária
        # após np.nan_to_num, mas não custa manter para máxima segurança)
        if not d_total_frete_carga.is_zero() and \
           d_total_frete_carga.is_finite() and not d_total_frete_carga.is_nan() and \
           d_valor_contratacao_carga.is_finite() and not d_valor_contratacao_carga.is_nan():
            
            # Linha que estava causando o erro, agora protegida por múltiplas verificações
            rentabilidade_percentual = ((d_total_frete_carga - d_valor_contratacao_carga) / d_total_frete_carga * Decimal('100')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            ...
            # Renomeado para evitar conflito com a função MAX_COST_PER_REGION
            max_cost_allowed_value = MAX_COST_PER_REGION.get(dominant_region, None) 
            
            if max_cost_allowed_value is not None:
                d_max_cost_allowed = Decimal(str(max_cost_allowed_value))

                custo_receita_ratio = (d_valor_contratacao_carga / d_total_frete_carga).quantize(Decimal('0.0001'), rounding=ROUND_HALF_UP)

                if custo_receita_ratio <= d_max_cost_allowed:
                    situacao_custo_regional = f"Dentro do Limite ({(d_max_cost_allowed * Decimal('100')).quantize(Decimal('1'), rounding=ROUND_HALF_UP)}%)"
                    cor_situacao = "#28a745"
                else:
                    situacao_custo_regional = f"Acima do Limite Região ({(d_max_cost_allowed * Decimal('100')).quantize(Decimal('1'), rounding=ROUND_HALF_UP)}%)"
                    cor_situacao = "#dc3545"
            else:
                situacao_custo_regional = f"Região '{dominant_region}' sem limite definido"
                cor_situacao = "orange"
        else: # d_total_frete_carga é Decimal('0') ou não é um número válido para cálculos
            rentabilidade_percentual = Decimal('0')
            if valor_contratacao_limpo > 0: # Usa o valor já limpo
                situacao_custo_regional = "Frete total zero, Contratação > 0"
                cor_situacao = "#dc3545"
            else:
                situacao_custo_regional = "Frete total zero, Contratação zero"
        ...
        # Verifica se o total do frete da carga é diferente de zero (usando tipo Decimal)
        if d_total_frete_carga != Decimal('0'):
            # Calcula a rentabilidade usando Decimal e arredonda
            rentabilidade_percentual = ((d_total_frete_carga - d_valor_contratacao_carga) / d_total_frete_carga * Decimal('100')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            ...
            if max_cost_allowed is not None:
                d_max_cost_allowed = Decimal(str(max_cost_allowed)) # Converte o percentual máximo para Decimal
                
                # Realiza a divisão e quantização com Decimais
                custo_receita_ratio = (d_valor_contratacao_carga / d_total_frete_carga).quantize(Decimal('0.0001'), rounding=ROUND_HALF_UP)
                ...
                situacao_custo_regional = f"Região '{dominant_region}' sem limite definido"
                cor_situacao = "orange"
        else: # total_frete_carga é zero (ou sua representação Decimal é zero)
            rentabilidade_percentual = Decimal('0')
            if valor_contratacao_carga > 0: # Se o frete é zero, mas a contratação não é
                situacao_custo_regional = "Frete total zero, Contratação > 0"
                cor_situacao = "#dc3545"
            else: # Ambos, frete e contratação, são zero
                situacao_custo_regional = "Frete total zero, Contratação zero"
                cor_situacao = "gray"
        ...
        with col2_placeholder:
            ...
            pdf_valor_contratacao = valor_contratacao_limpo # Use o valor_limpo aqui
            ...
            try:
                ...
                buffer_pdf = gerar_pdf_carga(
                    ...
                    placa=pdf_placa,
                    veiculo=pdf_veiculo,
                    valor_frete=total_frete_limpo, # Use o valor_limpo aqui
                    valor_contratacao=pdf_valor_contratacao
                )
