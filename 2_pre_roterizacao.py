...
def mover_entregas_para_carga_existente(ctrcs_selecionados, numero_carga_destino):
    try:
        if not ctrcs_selecionados:
            st.warning("⚠️ Nenhuma entrega selecionada para mover.")
            return
        if not numero_carga_destino or numero_carga_destino == "Selecionar uma Carga...":
            st.warning("⚠️ Selecione uma carga de destino válida.")
            return

        entregas_coletadas = []
        if ctrcs_selecionados:
            dados_pre_all = supabase.table("pre_roterizacao").select("*").in_("Serie_Numero_CTRC", ctrcs_selecionados).execute().data
            entregas_coletadas = dados_pre_all
        ...
        if dados_para_insercao:
            supabase.table("cargas_geradas").upsert(dados_para_insercao).execute()
            st.success(f"✅ {len(ctrcs_selecionados)} entrega(s) adicionada(s) à Carga {numero_carga_destino}.")

            if ctrcs_selecionados:
                supabase.table("pre_roterizacao").delete().in_("Serie_Numero_CTRC", ctrcs_selecionados).execute()
                st.info(f"Removidas {len(ctrcs_selecionados)} entregas de 'pre_roterizacao'.")
...
def pagina_gerenciar_usuarios():
    ...
    # ---- ALTERAÇÃO AQUI para incluir 'gestor' nas opções de criação ----
    class_options_create = ["colaborador", "aprovador", "gestor"]
    nova_classe = st.selectbox("Classe", class_options_create, key="classe_nova_criar")
# -------------------------------------------------------------------
    ...
    if not df.empty:
        ...
        # ---- ALTERAÇÃO AQUI para incluir 'gestor' nas opções de atualização ----
        options_for_edit_selectbox = ["colaborador", "aprovador", "gestor"]
        default_edit_index = 0
        if usuario_info["classe"] in options_for_edit_selectbox:
            default_edit_index = options_for_edit_selectbox.index(usuario_info["classe"])
        ...
        nova_classe_user = st.selectbox(
            "Nova classe",
            options=options_for_edit_selectbox, # <--- CORREÇÃO AQUI: Use a lista com 'gestor'
            index=default_edit_index,          # <--- CORREÇÃO AQUI: Use o índice calculado
            key=f"classe_edit_{usuario_alvo}" # Chave única para cada selectbox
        )
        novo_admin_status = st.checkbox("Administrador?", value=bool(usuario_info["is_admin"]))
...
def preparar_dados_estatisticas(df_combined):
    ...
    # Garante que a coluna 'Rota' esteja limpa e padronizada
    ...
    # GARANTE QUE COLUNAS CRÍTICAS EXISTAM E COM TIPO NUMÉRICO/STRING
    ...
    if 'valor_contratacao' not in df.columns: # Garante valor_contratacao exista para ponderação
        df['valor_contratacao'] = 0.0
    if 'Valor do Frete' not in df.columns: # Garante Valor do Frete exista
        df['Valor do Frete'] = 0.0
    if 'numero_carga' not in df.columns: # Garante numero_carga exista para agrupações
        df['numero_carga'] = df.index.astype(str) + '_CARGA_DESCONHECIDA' # Fallback para ID único por linha
...
def exportar_relatorio_estatisticas(df_stats_unfiltered, filtros_aplicados):
    ...
    df_filtered_for_export = df_stats_unfiltered[df_stats_unfiltered['mes_ano'] == mes_selecionado].copy() \
                             if mes_selecionado != "Todos os Meses" else df_stats_unfiltered.copy()
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Aba 1: Resumo Executivo (atualizada para incluir as novas colunas)
        if not df_filtered_for_export.empty:
            df_rotas_principais_for_export = preparar_dados_grid_rotas_principais(df_filtered_for_export)

            resumo = df_rotas_principais_for_export.groupby('Rota').agg(
                Qtd_de_Cargas=('Qtd de Cargas', 'first'), # Usar o 'first' para pegar o valor calculado por rota
                Qtd_de_Entregas=('Qtd de Entregas', 'first'),
                Custo_Ponderado=('Custo Percentual da Rota (Média Ponderada)', 'first'),
                Custo_Simples=('Custo Percentual da Rota (Média Simples)', 'first'),
                Custo_Medio_Ton=('Custo Médio por Tonelada', 'first')
            ).reset_index()

            resumo.rename(columns={
                'Custo_Ponderado': 'Custo Percentual (Ponderado)',
                'Custo_Simples': 'Custo Percentual (Simples)',
                'Custo_Medio_Ton': 'Custo Médio por Tonelada'
            }, inplace=True)
            ...
            worksheet.set_column('B:C', 15) # Qtd Cargas, Qtd Entregas
            worksheet.set_column('D:E', 25, percent_format) # Custos Ponderado/Simples
            worksheet.set_column('F:F', 25, currency_format) # Custo Ton
        else:
        ...
        # Aba 4: Detalhamento por Rota (atualizada para incluir as novas colunas)
        if not df_filtered_for_export.empty:
            detalhamento_rotas = df_rotas_principais_for_export.copy() # Reusa o df já preparado
            detalhamento_rotas.rename(columns={
                'Custo Percentual da Rota (Média Ponderada)': 'Custo_Ponderado',
                'Custo Percentual da Rota (Média Simples)': 'Custo_Simples',
                'Custo Médio por Tonelada': 'Custo_Medio_Ton'
            }, inplace=True)

            detalhamento_rotas['Custo_Ponderado_Texto'] = detalhamento_rotas['Custo_Ponderado'].apply(lambda x: f"{x:,.2f}%".replace('.', ','))
            detalhamento_rotas['Custo_Simples_Texto'] = detalhamento_rotas['Custo_Simples'].apply(lambda x: f"{x:,.2f}%".replace('.', ','))
            detalhamento_rotas['Custo_Medio_Ton_Texto'] = detalhamento_rotas['Custo_Medio_Ton'].apply(lambda x: f"R$ {formatar_brasileiro(x)}/ton")
            ...
            worksheet.set_column('A:A', 20) # Rota
            worksheet.set_column('B:C', 15) # Qtd Cargas, Qtd Entregas
            worksheet.set_column('D:E', 25, percent_format) # Custos
            worksheet.set_column('F:F', 25, currency_format) # Custo Ton
        else:
        ...
        if not df_filtered_for_export.empty:
            ...
            colunas_relevantes = [
                ...
                'mes_ano', 'valor_contratacao', 'Valor do Frete', 'justificativa_aprovacao_custo'
            ]
...
def preparar_dados_grid_rotas_principais(df_filtrado_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    Calcula: Qtde de Cargas, Qtde de Entregas, Custo Percentual da Rota (Média Ponderada),
             Custo Percentual da Rota (Média Simples), Custo Médio por Tonelada.
    """
    if df_filtrado_stats is None or df_filtrado_stats.empty:
        return pd.DataFrame(columns=[
            'Rota', 'Qtd de Cargas', 'Qtd de Entregas',
            'Custo Percentual da Rota (Média Ponderada)',
            'Custo Percentual da Rota (Média Simples)',
            'Custo Médio por Tonelada'
        ])

    df = df_filtrado_stats.copy()

    # Garante que as colunas necessárias existam e sejam numéricas/string e preenchidas
    numeric_cols = ['valor_contratacao', 'Peso Real em Kg', 'custo_percentual_frete', 'Valor do Frete']
    for col in numeric_cols:
        if col not in df.columns:
            df[col] = 0.0
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

    if 'Rota' not in df.columns: df['Rota'] = 'Rota Não Definida'
    if 'numero_carga' not in df.columns: df['numero_carga'] = df.index.astype(str) + '_CARGA_ND'
    if 'Serie_Numero_CTRC' not in df.columns: df['Serie_Numero_CTRC'] = df.index.astype(str) + '_ENTREGA_ND'
        
    df['Rota'] = df['Rota'].astype(str).str.strip().replace({'': 'Rota Não Definida', 'nan': 'Rota Não Definida', 'None': 'Rota Não Definida'})

    # 1. Contagens de Cargas e Entregas (do DF original, onde cada linha é uma entrega)
    ...
    # 2. Agregamos ao nível de CARGA ÚNICA DENTRO DE CADA ROTA
    # para obter os valores de contratação, frete, peso e o custo percentual por carga.
    df_cargas_unicas_por_rota = df.groupby(['Rota', 'numero_carga'], as_index=False).agg(
        valor_contratacao_carga=('valor_contratacao', 'first'),     # Valor de contratação da carga (já é único por carga)
        valor_frete_total_carga=('Valor do Frete', 'sum'),           # Soma do Valor do Frete das entregas (para essa carga)
        peso_real_kg_carga=('Peso Real em Kg', 'sum'),               # Soma do Peso Real das entregas (para essa carga)
        custo_percentual_frete_load=('custo_percentual_frete', 'first') # Custo % já calculado por carga
    )

    # 3. Agora, agregamos esses valores (de CARGAS ÚNICAS) ao nível da ROTA
    rota_somas_e_medias_data = df_cargas_unicas_por_rota.groupby('Rota', as_index=False).agg(
        Total_Valor_Contratacao_Rota=('valor_contratacao_carga', 'sum'),
        Total_Frete_Total_Cargas_Rota=('valor_frete_total_carga', 'sum'),
        Total_Peso_Real_Rota=('peso_real_kg_carga', 'sum'),
        Custo_Percentual_Simples_Rota=('custo_percentual_frete_load', 'mean') # Média simples dos custo_percentual_frete de cada carga
    )

    grid_data = pd.merge(grid_data, rota_somas_e_medias_data, on='Rota', how='left')

    # 4. Calcular Custo Percentual da Rota (Média Ponderada)
    grid_data['Custo Percentual da Rota (Média Ponderada)'] = grid_data.apply(
    ...
    # 5. Adicionar a Média Simples
    grid_data['Custo Percentual da Rota (Média Simples)'] = grid_data['Custo_Percentual_Simples_Rota'].fillna(0)

    # 6. Calcular Custo Médio por Tonelada
...
def pagina_cadastros():
    ...
    if st.session_state.crud_last_selected_table != selected_table:
        ...
        # ADICIONAR ESTA LINHA: Garante que a flag de carregamento para a nova tabela selecionada seja FALSE
        st.session_state.cadastros_data_loaded[selected_table] = False
        st.rerun()
    ...
    # === NOVO BLOCO: Botão de Atualizar Dados para a tabela selecionada ===
    col_button, col_message_area = st.columns([0.2, 0.8])
    with col_button:
        if st.button("🔄 Atualizar Dados", key=f"update_cadastros_data_button_{selected_table}"):
            st.session_state.cadastros_data_loaded[selected_table] = True
            load_table_data.clear() # Limpa o cache para load_table_data (força nova busca para a tabela)
            st.rerun()

    with col_message_area:
        if not st.session_state.cadastros_data_loaded.get(selected_table, False):
            st.info(f"Clique no botão 'Atualizar Dados' para carregar as informações da tabela '{selected_table}'.")
        else:
            st.markdown(
                f"""
                <div style="
                    ...
                    line-height: 1.5;
                ">
                    Dados de '{selected_table}' já carregados. Clique no botão para atualizar.
                </div>
                """,
                unsafe_allow_html=True
            )
    st.markdown("---") # Adiciona um separador após o botão/mensagem

    # --- Variável para controle de exibição do conteúdo da tabela ---
    should_display_table_content = st.session_state.cadastros_data_loaded.get(selected_table, False)

    # Chamar a função de UI específica para a tabela selecionada, passando o prefixo de chave E A NOVA FLAG
    if selected_table == "Rotas":
        gerenciar_rotas_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    if selected_table == "RotasPortoAlegre":
        gerenciar_rotas_pa_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    elif selected_table == "Clientes_Entrega_Agendada":
        gerenciar_clientes_agendados_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    elif selected_table == "Micro_Regiao_por_data_embarque":
        gerenciar_micro_regioes_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    elif selected_table == "Paletizadores":
        gerenciar_paletizadores_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    elif selected_table == "Particularidades":
        gerenciar_particularidades_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
    elif selected_table == "Motoristas":
        gerenciar_motorista_placa_ui(selected_table, TABLE_SCHEMAS[selected_table], supabase, current_key_prefix, should_display_table_content)
...
#
#---------------------------------------------------------------------------
def gerenciar_rotas_ui(table_name, schema, supabase_client, key_prefix, should_display_content): # NOVO PARÂMETRO
    st.header("Rotas ")
...
def gerenciar_rotas_pa_ui(table_name, schema, supabase_client, key_prefix="rotas_pa", should_display_content=False): 
...
def gerenciar_clientes_agendados_ui(table_name, schema, supabase_client, key_prefix="clientes_agendados", should_display_content=False): 
    """
...
def gerenciar_micro_regioes_ui(table_name, schema, supabase_client, key_prefix, should_display_content):
...
def gerenciar_paletizadores_ui(table_name, schema, supabase_client, key_prefix, should_display_content):
...
def gerenciar_particularidades_ui(table_name, schema, supabase_client, key_prefix, should_display_content):
...
def gerenciar_motorista_placa_ui(table_name, schema, supabase_client, key_prefix, should_display_content):
...
def pagina_aprovacao_diretoria():
    ...
    is_user_aprovador = (current_user_class == "aprovador" or current_user_class == "gestor")

    # Mensagem de aviso se o usuário não for aprovador
    if not is_user_aprovador:
        st.warning("⛔ Apenas usuários com classe 'aprovador' ou 'gestor' podem realizar ações de aprovação de diretoria.")
    ...
    if st.button("✅ Aprovar Todas as Entregas da Página", key="btn_aprovar_todas_topo", disabled=not is_user_aprovador):
        try:
...
def pagina_pre_roterizacao():
    ...
    with col_page_select:
        page_options_display_top = [f"Página {i+1}" for i in range(total_pages)]
        ...
        if selected_option_index_top != st.session_state.current_group_page_pre_roterizacao:
            st.session_state.current_group_page_pre_roterizacao = selected_option_index_top
            st.rerun()
        ...
        with st.expander(f"🔽 Selecionar entregas ({nome_grupo})", expanded=True):
                ...
                if st.button(
                    ...
                    if nova_rota == "Selecionar...":
                        ...
                        try:
                            ...
                            st.error(f"Erro ao mover entregas: {e}")
                if not rotas_disponiveis_para_movimentacao and not selecionadas.empty:
            ...
            with col_mover_carga_existente:
                ...
                if st.button(
                    ...
                    if carga_existente_selecionada == "Selecionar uma Carga...":
                        st.warning("Por favor, selecione uma carga válida para mover as entregas.")
                    else:
                        try:
                            if ctrcs_selecionados:
                                ctrcs_selecionados_lista = [s.get("Serie_Numero_CTRC") for s in selecionadas.to_dict(orient="records") if s.get("Serie_Numero_CTRC")]
...
def pagina_cargas_geradas():
    ...
    try:
        ...
        for carga in cargas_unicas:
            ...
            if 'is_reentrega_com_frete_pago' not in df_carga_raw.columns:
                df_carga_raw['is_reentrega_com_frete_pago'] = False
            df_carga_raw['is_reentrega_com_frete_pago'] = df_carga_raw['is_reentrega_com_frete_pago'].fillna(False).astype(bool)
            ...
            total_frete_real_coletado = df_carga_raw["Valor_do_Frete_Para_Calculo"].sum() + info_valor_adicional_raw
            
            # 2. Inicia a base de frete para a SUGESTÃO de valor com o frete real coletado
            total_frete_para_base_sugestao = total_frete_real_coletado

            # 3. Conta quantas entregas são de Retorno de Rota E são "gratuitas" (não reentrega com frete pago)
            free_return_deliveries_count = df_carga_raw[
                (df_carga_raw['is_retorno_rota'] == True) &
                (df_carga_raw['is_reentrega_com_frete_pago'] == False)
            ].shape[0]

            # 4. Se o frete real coletado for zero E existirem entregas de retorno gratuitas,
            #    ajustamos a base de frete *apenas para o cálculo da sugestão*, usando o valor assumido.
            if total_frete_real_coletado == 0.0 and free_return_deliveries_count > 0:
                total_frete_para_base_sugestao = free_return_deliveries_count * MIN_FRETE_ASSUMIDO_RETORNO_PARA_SUGESTAO 
            ...
            # --- INÍCIO DA MODIFICAÇÃO (Trecho 3/5) ---
            valor_sugerido_contratacao = 0.0
            # O cálculo agora usa 'total_frete_para_base_sugestao'
            if total_frete_para_base_sugestao > 0 and max_cost_allowed is not None:
                valor_sugerido_contratacao = total_frete_para_base_sugestao * max_cost_allowed
            ...
            with col1_badges:
                ...
                badge_valor_frete = badge(f'Valor frete: R$ {formatar_brasileiro(total_frete_real_coletado)}') 
                ...
                html_fragments.append(badge(f'Valor frete: R$ {formatar_brasileiro(total_frete_real_coletado)}', background_color='#333333', text_color='white'))
                        ...
                        try:
                            with st.spinner(f"Gerando PDF COMPLETO para a carga {carga}... Por favor, aguarde..."):
                                ...
                                buffer_pdf = gerar_pdf_carga(
                                    ...
                                    placa=pdf_placa,
                                    veiculo=pdf_veiculo,
                                    valor_frete=total_frete_real_coletado,
...
############################################# FIM DO BLOCO ###############################################################################
                ...
                with col_inputs_area:
                    ...
                    # --- INÍCIO DA MODIFICAÇÃO (Trecho 5/5) ---
                    elif total_frete_real_coletado > 0: # <-- AGORA USA O VALOR REAL
                        st.warning(f"Não foi possível calcular uma sugestão de valor de contratação para a Rota {rota_dominante} / Região {dominant_region}. Verifique o valor do frete e a região.")
                    else:
                        st.info("Não foi possível calcular uma sugestão de valor de contratação (frete total real zero e sem retornos gratuitos).")
                    ...
                    if st.button(
                        ...
                        try:
                            with st.spinner(" Retirando entregas da carga..."):
                                ...
                                # Limpa GrupoDeExibicao para que a lógica em pre_roterizacao possa reavaliar
                                # o Tipo_Tratativa_Especial (Paletizadas) ou a Rota.
                                if "GrupoDeExibicao" in df_para_retornar.columns:
                                    df_para_retornar["GrupoDeExibicao"] = None # Define como nulo para o banco de dados
                                else: # Se a coluna não existir, cria-a com None para consistência
                                    df_para_retornar["GrupoDeExibicao"] = None
                                # --- FIM DO NOVO BLOCO ---
...
# ==============================================================================
@st.cache_data(ttl=5400) # Cache para evitar consultas desnecessárias ao banco
...
def pagina_aprovacao_custos():
    ...
    try: # <--- TRY PRINCIPAL DA FUNÇÃO
        ...
        for carga in cargas_unicas:
            ...
            # --- NOVO: Garante que 'is_reentrega_com_frete_pago' exista e seja booleano ---
            if 'is_reentrega_com_frete_pago' not in df_carga.columns:
...
def pagina_cargas_aprovadas():
    ...
    try:
        ...
        numeric_cols_to_convert = [
            'Peso Real em Kg', 'Peso Calculado em Kg', 'Cubagem em m³',
            'Quantidade de Volumes', 'Valor do Frete', 'valor_contratacao',
            'custo_percentual_frete', 'rentabilidade_percentual', 'valor_adicional_frete' 
        ]
        for col in numeric_cols_to_convert:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
        ...
        for carga in cargas_unicas:
            ...
            # --- NOVO BLOCO: PREPARAR O VALOR DO FRETE AJUSTADO PARA CÁLCULOS (ESSENCIAL) ---
            # Este bloco é necessário para garantir que os valores dos badges sejam precisos,
            # mesmo que os valores de custo/rentabilidade já venham do DB.
            ...
            # 3. Cálculos de Frete para exibição nos Badges
            total_frete_base_bruto = df_carga["Valor do Frete"].sum() # SOMA BRUTA
            total_frete_carga = df_carga["Valor_do_Frete_Para_Calculo"].sum() # SOMA AJUSTADA (LÍQUIDA)
            # --- FIM DO BLOCO DE CÁLCULO AJUSTADO ---
            ...
            # Valores de custo/rentabilidade já salvos no DB
            custo_percentual_frete_db = df_carga["custo_percentual_frete"].iloc[0] if "custo_percentual_frete" in df_carga.columns and pd.notna(df_carga["custo_percentual_frete"].iloc[0]) else 0.0
            rentabilidade_percentual_db = df_carga["rentabilidade_percentual"].iloc[0] if "rentabilidade_percentual" in df_carga.columns and pd.notna(df_carga["rentabilidade_percentual"].iloc[0]) else 0.0
            situacao_custo_regional_db = df_carga["situacao_custo_regional"].iloc[0] if "situacao_custo_regional" in df_carga.columns and pd.notna(df_carga["situacao_custo_regional"].iloc[0]) else 'N/A'
            
            # --- CÁLCULO DOS TOTAIS (PARA BADGES) ---
            total_frete_bruto = total_frete_base_bruto + info_valor_adicional_raw
            total_frete_limpo = total_frete_carga + info_valor_adicional_raw # Frete Líquido (Ajustado)
            ...
            with col1_badges:
                ...
                # *** NOVO BADGE: VALOR BRUTO (REFERÊNCIA) ***
                all_badges_html_list.append(badge(f'💰 Frete Bruto (Ref Cargas Geradas): R$ {formatar_brasileiro(total_frete_bruto)}', background_color='#6c757d', text_color='white'))
                    ...
                    try:
                        with st.spinner(f"Gerando PDF para a carga {carga}... Por favor, aguarde..."):
                            ...
                            buffer_pdf = gerar_pdf_carga(
                                ...
                                placa=pdf_placa,
                                veiculo=pdf_veiculo,
                                valor_frete=total_frete_limpo, # Deve usar o valor líquido (ajustado) no PDF
                                valor_contratacao=pdf_valor_contratacao
                            )
                    ...
                    if st.button(
                        ...
                        try:
                            with st.spinner(f"Retornando carga {carga} para aprovação de custos..."):
                                ...
                                if records_to_insert:
                                    supabase.table("aprovacao_custos").upsert(records_to_insert).execute()
                                else:
...
def pagina_estatisticas():
    ...
    # Inicializações existentes de st.session_state
    ...
    if 'metrica_custo_bar_chart' not in st.session_state:
        st.session_state.metrica_custo_bar_chart = 'Custo Percentual da Rota (Média Ponderada)'
    
    # --- INÍCIO DA MODIFICAÇÃO: Botão de Carregar/Atualizar Dados (Trecho 1/1) ---
    col_button, col_status = st.columns([0.15, 1.5])

    with col_button:
        # O botão força o carregamento dos dados
        if st.button("🔄 Buscar Dados Atualizados", key="update_stats_data_button"):
            st.session_state.estatisticas_data_loaded = True # Marca para carregar/recarregar
            _get_cargas_fechadas_data.clear() # Limpa o cache da função de dados cacheada para forçar nova consulta
            # O rerun será feito abaixo, se os dados ainda não estiverem carregados ou se o botão for clicado.

    # Carrega dados APENAS se o botão foi clicado OU se os dados ainda não foram carregados
    if not st.session_state.estatisticas_data_loaded or st.session_state.estatisticas_df_cache.empty:
        with st.spinner("🔄 Carregando dados para análise estatística (Isso pode demorar um pouco)..."):
            try:
                # Usa a função cacheada para carregar os dados
                df_cargas_fechadas_full = _get_cargas_fechadas_data() 
                
                if df_cargas_fechadas_full.empty:
                    st.info("ℹ️ Não há dados suficientes em 'cargas_fechadas' para gerar estatísticas.")
                    st.session_state.estatisticas_df_cache = pd.DataFrame()
                    st.session_state.estatisticas_data_loaded = False # Reinicia o flag
                    # Não precisamos fazer `return` aqui, mas podemos exibir uma mensagem.
                    # O resto da função vai lidar com df_stats vazio.

                else:
                    st.session_state.estatisticas_df_cache = df_cargas_fechadas_full
                    st.session_state.estatisticas_data_loaded = True # Marca como carregado
                    st.toast("Dados de estatísticas carregados com sucesso!", icon="✅")
                
                # Após carregar o DataFrame completo, processa ele para df_stats (que já inclui as colunas calculadas)
                st.session_state.df_stats = preparar_dados_estatisticas(st.session_state.estatisticas_df_cache)
                
                # Se o botão foi clicado, force um rerun para exibir os dados e gráficos.
                if st.session_state.estatisticas_data_loaded:
                     st.rerun() # Force um rerun para que os filtros e gráficos sejam atualizados imediatamente.

            except Exception as e:
                st.error(f"❌ Erro ao carregar dados para estatísticas: {e}")
                st.exception(e)
                st.session_state.estatisticas_data_loaded = False # Em caso de erro, permite tentar novamente
                st.session_state.estatisticas_df_cache = pd.DataFrame() # Limpa o cache para não usar dados incompletos
                st.session_state.df_stats = pd.DataFrame()
                st.session_state.df_filtrado_stats = pd.DataFrame()
                return # Sai da função para evitar erros com DataFrames vazios
    else:
        # Se os dados já estão carregados, apenas use-os do cache
        st.session_state.df_stats = preparar_dados_estatisticas(st.session_state.estatisticas_df_cache)
        with col_status: # Esta coluna agora é mais estreita
            # --- INÍCIO DA MODIFICAÇÃO (Trecho 1/1) ---
            st.markdown(
                """
                <div style="
                    display: inline-block;
    ...
    with col_filtros1:
        ...
        selected_mes_stats = st.selectbox(
            "   Selecionar Mês/Ano:",
            options=["Todos os Meses"] + meses_disponiveis,
            key="filtro_mes_stats"
        )
        if selected_mes_stats != st.session_state.mes_selecionado_stats:
            st.session_state.mes_selecionado_stats = selected_mes_stats
            st.rerun()
    ...
    with tab1:
        ...
        if not df_rotas_principais.empty:
            ...
            with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                ...
                df_export_formatted['Custo Percentual da Rota (Média Ponderada)'] = df_export_formatted['Custo Percentual da Rota (Média Ponderada)'].apply(lambda x: f"{x:,.2f}%".replace('.', ','))
                df_export_formatted['Custo Percentual da Rota (Média Simples)'] = df_export_formatted['Custo Percentual da Rota (Média Simples)'].apply(lambda x: f"{x:,.2f}%".replace('.', ','))
            ...
            st.markdown("<small><i>Clique no nome da rota para expandir e ver os detalhes das justificativas.</i></small>", unsafe_allow_html=True)
            st.markdown("---") # Separador visual

            # Iterar sobre cada rota para exibir as informações colunadas e o expander
            for idx, row in df_rotas_principais.iterrows():
                # NOVO: Criamos o st.expander que encapsulará os detalhes da rota
                with st.expander(
                    f"Rota: {row['Rota']} | Cargas: {int(row['Qtd de Cargas'])} | "
                    f"Entregas: {int(row['Qtd de Entregas'])} | "
                    f"Custo Ponderado: {row['Custo Percentual da Rota (Média Ponderada)']:.2f}% | "
                    f"Custo Simples: {row['Custo Percentual da Rota (Média Simples)']:.2f}% | "
                    f"Custo/Ton: R$ {formatar_brasileiro(row['Custo Médio por Tonelada'])}/ton", 
                    expanded=False
                ):
                    
                    # Conteúdo interno do expander (restante da lógica do detalhamento da justificativa)
                    route_name_for_display = row['Rota']
                    st.markdown(f"##### Detalhamento das Justificativas para Rota: {route_name_for_display}")

                    df_justificativas_rota_completo = st.session_state.df_filtrado_stats[
                        st.session_state.df_filtrado_stats['Rota'] == route_name_for_display
                    ].copy()
                    ...
                    if not df_justificativas_detalhe.empty:
                        total_cargas_na_rota_for_display = df_justificativas_detalhe['Qtd de Cargas'].sum()
                        total_entregas_na_rota_for_display = df_justificativas_detalhe['Qtd de Entregas'].sum()

                        if total_cargas_na_rota_for_display > 0:
                            df_justificativas_detalhe['% sobre Total de Cargas na Rota'] = (
                                df_justificativas_detalhe['Qtd de Cargas'] / total_cargas_na_rota_for_display * 100
                            ).round(1)
                        else:
                            df_justificativas_detalhe['% sobre Total de Cargas na Rota'] = 0.0

                        if total_entregas_na_rota_for_display > 0:
                            df_justificativas_detalhe['% sobre Total de Entregas na Rota'] = (
                                df_justificativas_detalhe['Qtd de Entregas'] / total_entregas_na_rota_for_display * 100
                            ).round(1)
                        else:
                            df_justificativas_detalhe['% sobre Total de Entregas na Rota'] = 0.0

                        df_display_for_native_dataframe = df_justificativas_detalhe[[
                            'Justificativas', 
                            'Qtd de Cargas', 
                        ...
                        # --- 2. ALTERADO: Configuração para st.dataframe ---
                        column_config_just = {
                            "Justificativas": st.column_config.TextColumn("Justificativa", width="large"),
                            "Qtd de Cargas": st.column_config.NumberColumn("Qtd. Cargas", format="%d", help="Quantidade de cargas únicas"),
                            "% sobre Total de Cargas na Rota": st.column_config.NumberColumn("% Total Cargas", format="%.1f%%", help="Percentual sobre o total de cargas na rota"),
                            "Qtd de Entregas": st.column_config.NumberColumn("Qtd. Entregas", format="%d", help="Quantidade total de entregas"),
                            "% sobre Total de Entregas na Rota": st.column_config.NumberColumn("% Total Entregas", format="%.1f%%", help="Percentual sobre o total de entregas na rota"),
                            "% Custo Justificativa na Rota": st.column_config.NumberColumn("% Custo Justif.", format="%.1f%%", help="Percentual do custo da justificativa sobre o custo total da rota"),
                        }

                        st.dataframe(
                            df_display_for_native_dataframe,
                            use_container_width=True,
                            hide_index=True,
                            column_config=column_config_just
                        )

                        # Export button for the inline content
                        excel_buffer_just_inline = io.BytesIO()
                        with pd.ExcelWriter(excel_buffer_just_inline, engine='xlsxwriter') as writer_just_inline:
                            df_export_inline = df_justificativas_detalhe[[
                                'Justificativas', 
                                'Qtd de Cargas', 
                                '% sobre Total de Cargas na Rota',
                                'Qtd de Entregas',
                            ...
                            df_export_inline['% sobre Total de Cargas na Rota'] = df_export_inline['% sobre Total de Cargas na Rota'].apply(lambda x: f"{x:,.1f}%".replace('.', ','))
                            df_export_inline['% sobre Total de Entregas na Rota'] = df_export_inline['% sobre Total de Entregas na Rota'].apply(lambda x: f"{x:,.1f}%".replace('.', ','))
                            df_export_inline['% Custo Justificativa na Rota'] = df_export_inline['% Custo Justificativa na Rota'].apply(lambda x: f"{x:,.1f}%".replace('.', ','))

                            df_export_inline.to_excel(writer_just_inline, sheet_name=f"Just_{route_name_for_display}", index=False)
                        excel_buffer_just_inline.seek(0)

                        st.download_button(
                            label=f"⬇️ Exportar Detalhes de Justificativas ({route_name_for_display})",
                            data=excel_buffer_just_inline,
                            file_name=f"justificativas_{route_name_for_display}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"export_just_details_inline_{route_name_for_display}"
                        )

                    else:
                        st.info(f"Nada a ser mostrado para a rota **{route_name_for_display}** (nenhuma justificativa encontrada).")
                    
                    st.markdown("---") # Separador visual dentro do expander

        else: # if df_rotas_principais.empty
            st.info("Nenhuma rota encontrada para exibir o resumo principal.")

        st.markdown("--- ") # Separador geral
        
        # ... (rest of tab1 content remains the same - charts) ...
        if not st.session_state.df_filtrado_stats.empty:
            # Agregação por carga para os gráficos
            df_loads_for_charts = st.session_state.df_filtrado_stats.groupby('numero_carga').agg(
                Rota=('Rota', 'first'),
                custo_percentual_frete=('custo_percentual_frete', 'first'),
                valor_contratacao=('valor_contratacao', 'first'), # Inclui para calcular ponderada aqui
                Valor_do_Frete=('Valor do Frete', 'sum'), # Soma o frete por carga
            ...
            if not df_loads_for_charts.empty and 'data_fechamento' in df_loads_for_charts.columns:
                ...
                # Cálculo da média ponderada para o gráfico de barras aqui também
                df_loads_for_charts['valor_contratacao'] = pd.to_numeric(df_loads_for_charts['valor_contratacao'], errors='coerce').fillna(0)
                df_loads_for_charts['Valor_do_Frete'] = pd.to_numeric(df_loads_for_charts['Valor_do_Frete'], errors='coerce').fillna(0)

                df_loads_for_charts['custo_calculado_ponderado'] = df_loads_for_charts.apply(
                    lambda r: (r['valor_contratacao'] / r['Valor_do_Frete'] * 100) if r['Valor_do_Frete'] > 0 else 0,
                    axis=1
                )
                ...
                if not df_loads_for_charts.empty:
                    ...
                    # Calcula a média simples do custo percentual por rota e período
                    df_daily_avg_cost_simples = df_loads_for_charts.groupby(['Rota', 'Periodo'], as_index=False)['custo_percentual_frete'].mean()
                    # Calcula a média ponderada do custo percentual por rota e período
                    df_daily_avg_cost_ponderado = df_loads_for_charts.groupby(['Rota', 'Periodo'], as_index=False).agg(
                        Total_Valor_Contratacao=('valor_contratacao', 'sum'),
                        Total_Valor_Frete=('Valor_do_Frete', 'sum')
                    )
                    df_daily_avg_cost_ponderado['custo_calculado_ponderado'] = df_daily_avg_cost_ponderado.apply(
                        lambda row: (row['Total_Valor_Contratacao'] / row['Total_Valor_Frete'] * 100) if row['Total_Valor_Frete'] > 0 else 0.0,
                        axis=1
                    )
                    df_daily_avg_cost_ponderado.drop(columns=['Total_Valor_Contratacao', 'Total_Valor_Frete'], inplace=True)
                    df_daily_avg_cost_ponderado.rename(columns={'custo_calculado_ponderado': 'custo_percentual_frete'}, inplace=True)

                    # Merge as duas para ter ambas opções
                    df_daily_avg_cost = pd.merge(df_daily_avg_cost_simples, df_daily_avg_cost_ponderado, on=['Rota', 'Periodo'], suffixes=('_simples', '_ponderado'))
                    ...
                    col_metrica_linha, _ = st.columns([0.4, 0.6])
                    with col_metrica_linha:
                        metrica_line_chart = st.selectbox(
                            "Escolha a métrica para o gráfico de linha:",
                            options=['custo_percentual_frete_simples', 'custo_percentual_frete_ponderado'],
                            format_func=lambda x: "Custo Simples" if x == 'custo_percentual_frete_simples' else "Custo Ponderado",
                            key="metrica_line_chart"
                        )
                    if not df_daily_avg_cost.empty:
                        fig_line = px.line(
                            df_daily_avg_cost,
                            x='Periodo',
                            y=metrica_line_chart, # Usando a métrica selecionada
                            color='Rota',
                            title="Evolução Diária do Custo Percentual Médio por Rota",
                            labels={'Periodo': x_axis_label, metrica_line_chart: 'Custo Médio (%)'},
                            hover_name='Rota',
                            line_shape='spline',
                            markers=True
                        )
                        ...
                        fig_line.update_traces(visible='legendonly') # Inicia com as linhas ocultas
                    ...
                    col_metrica_barras, _ = st.columns([0.4, 0.6])
                    with col_metrica_barras:
                        metrica_bar_chart = st.selectbox(
                            "Escolha a métrica para o gráfico de barras:",
                            options=['Custo Percentual da Rota (Média Simples)','Custo Percentual da Rota (Média Ponderada)' ],
                            key="metrica_bar_chart"
                        )

                    # Ajuste aqui para usar o df_rotas_principais que já tem as duas métricas
                    df_overall_avg_cost_for_bar = df_rotas_principais.copy()
                    df_overall_avg_cost_for_bar = df_overall_avg_cost_for_bar.sort_values(by=metrica_bar_chart, ascending=False)
                    
                    if not df_overall_avg_cost_for_bar.empty:
                        # 1) Limite do eixo X com margem
                        max_cost_value = float(df_overall_avg_cost_for_bar[metrica_bar_chart].max())
                        x_axis_upper_limit = max(10.0, max_cost_value * 1.10) # 10% acima do máximo

                        # 2) Altura dinâmica
                        num_routes = len(df_overall_avg_cost_for_bar)
                        chart_height = min(max(350, num_routes * 28), 900) # Mínimo 350px, até 900px

                        fig_bar = px.bar(
                            df_overall_avg_cost_for_bar,
                            x=metrica_bar_chart, # Usando a métrica selecionada
                            y='Rota',
                            orientation='h',
                            title="Custo Percentual Médio por Rota",
                            labels={metrica_bar_chart: 'Custo Médio (%)', 'Rota': 'Rota'},
                            color=metrica_bar_chart,
                            color_continuous_scale=px.colors.diverging.RdYlGn_r, 
                            text=metrica_bar_chart
                        )
                        ...
                        fig_bar.update_layout(
                            ...
                            bargap=0.15, # Barras mais "grossas"
                            margin=dict(l=10, r=40, t=60, b=10) # Margem para texto outside
                        )
                        fig_bar.update_xaxes(range=[0, x_axis_upper_limit], ticksuffix="%")
...
@st.cache_data(ttl=5400) # Cache os dados por 15 minutos (900 segundos)
def _get_cargas_fechadas_data():
    ...
    try:
        ...
        # --- NOVO: Garante que 'is_reentrega_com_frete_pago' exista e seja booleano ---
        if 'is_reentrega_com_frete_pago' not in df_original.columns:
            df_original['is_reentrega_com_frete_pago'] = False
        df_original['is_reentrega_com_frete_pago'] = df_original['is_reentrega_com_frete_pago'].fillna(False).astype(bool)
        # --- FIM NOVO ---
...
def pagina_cargas_fechadas():
    ...
    with col_clear_cache_button:
        if st.button("Atualizar Dados", key="clear_cargas_fechadas_cache"):
            _get_cargas_fechadas_data.clear() # Limpa o cache da função de dados cacheada.
            # MODIFICADO/ADICIONADO: Define a flag como True para carregar dados na próxima execução.
            st.session_state.cargas_fechadas_data_loaded = True
            
            st.rerun() # Força o Streamlit a re-executar o script.
    
    df_original = pd.DataFrame() # Inicializa como DataFrame vazio para garantir que sempre esteja definido.

    # MODIFICADO/ADICIONADO: Os dados só serão carregados se a flag 'cargas_fechadas_data_loaded' for True.
    if st.session_state.cargas_fechadas_data_loaded:
        try:
            with st.spinner("Carregando dados para cargas encerradas..."):
                df_original = _get_cargas_fechadas_data() # Esta chamada agora é condicional.
                
            if df_original.empty:
                st.info("Nenhuma carga foi fechada ainda ou não há dados nos últimos 30 dias.")
                # MODIFICADO/ADICIONADO: Se o DataFrame estiver vazio após o carregamento (e possível filtragem),
                # resetamos a flag para que o usuário precise clicar novamente.
                st.session_state.cargas_fechadas_data_loaded = False
                return # Interrompe a execução se não houver dados.

        except Exception as e:
            st.error(f"Erro CRÍTICO ao carregar cargas fechadas: {e}")
            st.exception(e)
            st.session_state.cargas_fechadas_data_loaded = False # Resetar em caso de erro.
            return # Interrompe a execução em caso de erro.
    else:
        # MODIFICADO/ADICIONADO: Se a flag for False (estado inicial ou após um reset), instrui o usuário.
        st.info("Clique no botão 'Atualizar Dados' para carregar as informações das cargas encerradas.")
        return 
    
    # --- NOVO FILTRO GLOBAL: Últimos 30 dias ---
    hoje = datetime.now(FUSO_BRASIL).date()
    data_30_dias_atras = hoje - timedelta(days=30)
    
    # Filtra df_original. Certifica-se que 'data_fechamento' é datetime e tz-aware para comparação
    if 'data_fechamento' in df_original.columns:
        # Garante que data_fechamento é datetime e tz-aware no fuso horário do Brasil para o filtro
        df_original['data_fechamento_local'] = df_original['data_fechamento'].dt.tz_convert(FUSO_BRASIL)
        df_original = df_original[df_original['data_fechamento_local'].dt.date >= data_30_dias_atras].copy()
        df_original.drop(columns=['data_fechamento_local'], inplace=True) # Remove a coluna auxiliar
    else:
        st.warning("Coluna 'data_fechamento' não encontrada para aplicar filtro de 30 dias. Exibindo todos os dados.")
        
    # Após aplicar o filtro, verifica novamente se o DataFrame ficou vazio
    if df_original.empty:
        st.info(f"Nenhuma carga encontrada para os últimos 30 dias (a partir de {hoje.strftime(DATE_ONLY_DISPLAY_FORMAT_STRING_BR)}).")
        return

    st.markdown(
    f"""
    <div style="
        display: inline-block; /* Faz com que o div ocupe apenas a largura do seu conteúdo */
        max-width: 100%; /* Garante que não exceda a largura da coluna pai se o texto for muito longo */
        background-color: #E6F7FF; /* Cor de fundo azul claro, similar ao st.info padrão */
        border-left: 0.25rem solid #2196F3; /* Borda esquerda azul, similar ao st.info padrão */
        color: #262730; /* Cor do texto escuro */
        padding: 0.75rem 1.25rem; /* Espaçamento interno */
        border-radius: 0.25rem; /* Cantos arredondados */
        margin-bottom: 1rem; /* Margem inferior para espaçamento, ajuste se necessário */
    ">
        Exibindo cargas encerradas dos últimos 30 dias (a partir de {hoje.strftime(DATE_ONLY_DISPLAY_FORMAT_STRING_BR)}). Utilize os filtros abaixo para refinar.
    </div>
    """,
    unsafe_allow_html=True
)
    # --- FIM NOVO FILTRO GLOBAL ---
    # --- INICIALIZAÇÃO E APLICAÇÃO DO FILTRO DE NÚMERO DA CARGA ---
    df_filtered = df_original.copy() # df_filtered agora começa já com os últimos 30 dias (ou menos, se o df_original estava vazio)
    ...
    try:
        # PASSA df_original (TODOS OS DADOS) para aplicar a formatação correta ANTES de gerar o CSV
        # ATENÇÃO: É df_original aqui, não df_filtered!
        df_csv_geral = df_original[[col for col in colunas_para_csv_geral_completo if col in df_original.columns]].copy()

        # ... (loop de formatação para as colunas de df_csv_geral) ...
        # ESTE LOOP DE FORMATAÇÃO DEVE SER MANTIDO IGUAL, POIS AGORA ELE OPERA SOBRE df_original
        # CERTIFIQUE-SE QUE ESTE BLOCO ESTEJA DENTRO DO SEU TRY/EXCEPT CORRETO
        for col in df_csv_geral.columns:
            if col in GLOBAL_DB_TIMESTAMP_COLS:
                ...
                # Adiciona aspas duplas internas para proteger valores com hífen ou espaços no Excel
                df_csv_geral[col] = df_csv_geral[col].astype(str).str.strip().apply(lambda x: f'"{x}"' if x else '')
            elif col == 'is_retorno_rota':  # Formatação para booleano
                df_csv_geral[col] = df_csv_geral[col].apply(lambda x: 'Sim' if x else 'Não')
            else:
        ...
        with col_download_placeholder:
            st.download_button(
                    label="⬇️ Baixar CSV Completo", # RENOMEADO O LABEL DO BOTÃO
                    data=df_csv_geral.to_csv(index=False, sep=';', encoding='utf-8-sig'),
                    file_name="cargas_fechadas_COMPLETO.csv", # RENOMEADO O NOME DO ARQUIVO
                    mime="text/csv",
                    key="download_csv_geral_completo" # RENOMEADA A KEY DO BOTÃO
                )
    except Exception as e:
        st.warning(f"⚠️ Não foi possível gerar o CSV completo: {e}")
        st.exception(e)
    st.markdown("---")
...
if "search_ctrc_input_value" not in st.session_state:
    st.session_state.search_ctrc_input_value = "" # Inicialize com uma string vazia
...
# === NOVO BLOCO: Variáveis de estado para controlar o diálogo de reentrega ===
if 'show_reentrega_dialog' not in st.session_state:
    st.session_state.show_reentrega_dialog = {} # Dicionário: {nome_grupo: True/False}
if 'reentrega_response' not in st.session_state:
    st.session_state.reentrega_response = {} # Dicionário: {nome_grupo: "Sim"/"Não"/None}
if 'reentrega_frete_input_value' not in st.session_state:
    st.session_state.reentrega_frete_input_value = {} # Dicionário: {nome_grupo: valor}
if 'ctrcs_for_dialog' not in st.session_state:
    st.session_state.ctrcs_for_dialog = {} # Dicionário: {nome_grupo: [ctrcs]}
if 'cargo_num_for_dialog' not in st.session_state:
    st.session_state.cargo_num_for_dialog = {} # Dicionário: {nome_grupo: numero_carga}
if 'current_group_page_pre_roterizacao' not in st.session_state:
    st.session_state.current_group_page_pre_roterizacao = 0 # Inicia na primeira página (índice 0)

if 'estatisticas_data_loaded' not in st.session_state:
    st.session_state.estatisticas_data_loaded = False # Indica se os dados de estatísticas já foram carregados
if 'estatisticas_df_cache' not in st.session_state:
    st.session_state.estatisticas_df_cache = pd.DataFrame() # Cache para o DataFrame completo (sem filtro de mês)

if 'cargas_fechadas_data_loaded' not in st.session_state:
    st.session_state.cargas_fechadas_data_loaded = False

if 'cadastros_data_loaded' not in st.session_state:
    st.session_state.cadastros_data_loaded = {}
# === FIM NOVO BLOCO ===
# ========== EXECUÇÃO PRINCIPAL ========== #
