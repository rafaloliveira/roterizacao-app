    ...
    for cliente_pagador in clientes_pagadores_unicos:
        ...
        with col_badge:
            ...
            st.markdown(
                ...
                f"<span style='background:#FF4500;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>🚫 Agendas sem Data Prog.: {qtd_agendar_sem_programacao_cliente}</span>"
                f"<span style='background:#DAA520;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>⚠️ Com Particularidade: {qtd_com_particularidade_cliente}</span>",
                unsafe_allow_html=True
            )
...
""")
    ...
    for cliente in sorted(df_aprovacao["Cliente Pagador"].unique()): # df_aprovacao já definido aqui
        ...
        with col_badge:
            ...
            st.markdown(
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>{len(df_cliente)} entregas</span>" +
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>{formatar_brasileiro(df_cliente['Peso Calculado em Kg'].sum())} kg calc</span>" +
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>{formatar_brasileiro(df_cliente['Peso Real em Kg'].sum())} kg real</span>" +
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>Valor frete: R\$ {formatar_brasileiro(df_cliente['Valor do Frete'].sum())}</span>" +
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>{formatar_brasileiro(df_cliente['Cubagem em m³'].sum())} m³</span>" +
                f"<span style='background:#eef2f7;border-radius:12px;padding:6px 12px;margin:4px;color:inherit;display:inline-block;'>{int(df_cliente['Quantidade de Volumes'].sum())} volumes</span>" +
                f"<span style='background:#FF4500;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>🚫 Agendas sem Data Prog.:{qtd_agendar_sem_programacao_cliente}</span>" +
                f"<span style='background:#DAA520;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>⚠️ Com Particularidade: {qtd_com_particularidade_cliente}</span>",
                unsafe_allow_html=True
            )
...
""")
    ...
    for rota_visual in sorted(df_visivel["Rota_Grupo"].dropna().unique()):
        ...
        st.markdown(
            ...
            f"<span style='background:#FF4500;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>🚫 Agendas sem Data Prog.: {qtd_agendar_sem_programacao_rota}</span>" +
            f"<span style='background:#DAA520;border-radius:12px;padding:6px 12px;margin:4px;color:white;display:inline-block;'>⚠️ Com Particularidade: {qtd_com_particularidade_rota}</span>",
            unsafe_allow_html=True
        )
        ...
        for carga in cargas_unicas:
            ...
            with col1_badges:
                ...
                html_fragments = [
                    ...
                    badge_motorista,
                    badge_placa,
                    badge_valor_contratacao_badge,
                    pdf_badge_html_fragment,
                    badge_percentual_custo,
                    badge(f'🚫 Agendas sem Data Prog.: {qtd_agendar_sem_programacao_carga}', background_color="#FF4500", text_color="white"),
                    badge(f'⚠️ Com Particularidade: {qtd_com_particularidade_carga}', background_color="#DAA520", text_color="white")
                ]

                if info_data_hora_gerada:
                    html_fragments.append(badge(f'Gerada em: {formatar_data_hora_br(info_data_hora_gerada)}'))
    ...
    try: # <--- TRY PRINCIPAL DA FUNÇÃO
        ...
        for carga in cargas_unicas:
            ...
            with col1_badges:
                ...
                all_badges_html_list.append(badge(f'Valor Contratação: R$ {formatar_brasileiro(valor_contratacao_carga_existente)}'))
                all_badges_html_list.append(badge(f'Motorista: {motorista_}'))
                all_badges_html_list.append(badge(f'Placa: {placa_veiculo}'))
                all_badges_html_list.append(badge(f'Veículo: {veiculo}'))
                all_badges_html_list.append(badge(f'Gerada em: {formatar_data_hora_br(info_data_hora_gerada)}'))
                # Adicione o badge da Data_Hora_Gerada APENAS se houver valor
                
                    

                # Adicione os badges específicos da página de Aprovação de Custos
               ...
               # Novos cálculos por carga
                ...
                all_badges_html_list.append(badge(f'🚫 Agendas sem Data Prog.: {qtd_agendar_sem_programacao_carga}', background_color="#FF4500", text_color="white"))
                all_badges_html_list.append(badge(f'⚠️ Com Particularidade: {qtd_com_particularidade_carga}', background_color="#DAA520", text_color="white"))
    ...
    try:
        ...
        for carga in cargas_unicas:
            ...
            with col1_badges:
                ...
                all_badges_html_list.append(badge(f'Gerada em: {formatar_data_hora_br(info_data_hora_gerada)}'))
                # Adicione os badges específicos da página de Cargas Aprovadas
                all_badges_html_list.append(badge(f'Custo: {percentual_custo_texto}', background_color=cor_badge_custo, text_color=cor_texto_custo))
                all_badges_html_list.append(badge(f'Rentabilidade: {rentabilidade_percentual:.2f}%'))
                ...
                # Verifique se data_aprovacao_custos existe e não é nula antes de tentar formatar
                ...
                if justificativa_aprovacao:
                    all_badges_html_list.append(badge(f'Justificativa: {justificativa_aprovacao}', background_color='#FFD700', text_color='#333'))
...
def pagina_cargas_fechadas():
    ...
    for carga in cargas_unicas_paginated:
        ...
        with col1_badges:
            ...
            all_badges_html_list.append(badge(f'Situação Custo: {situacao_custo_regional}', background_color=cor_situacao, text_color='white'))
            all_badges_html_list.append(badge(f'Aprovado por: {aprovador_custos_login}'))
            all_badges_html_list.append(badge(f'Aprovado em: {data_aprovacao_custos_display}'))
            all_badges_html_list.append(badge(f'Fechado por: {fechador_carga_login}'))
            all_badges_html_list.append(badge(f'Fechado  em: {data_fechamento_carga_display}', background_color="#2c2c2e", text_color="white"))


         
                
            if data_hora_gerada_display:
                all_badges_html_list.append(badge(f'Gerada em: {data_hora_gerada_display}'))
            if pdf_downloaded_at_display:
                all_badges_html_list.append(badge(f'PDF baixado em: {pdf_downloaded_at_display}', background_color="#6c757d", text_color="white"))
           
            if csv_downloaded_at_display:
                all_badges_html_list.append(badge(f'CSV baixado em: {csv_downloaded_at_display}', background_color="#6c757d", text_color="white"))
