...
def pagina_pre_roterizacao():
    ...
    for rota in sorted(df["Rota"].dropna().unique()):
        ...
        # ✅ CORREÇÃO PRINCIPAL: Configurações otimizadas para barra de rolagem
        ...
        # ✅ CORREÇÃO PRINCIPAL: Removida largura fixa e adicionado CSS customizado
        grid_response = AgGrid(
            df_formatado,
            gridOptions=grid_options,
            update_mode=GridUpdateMode.SELECTION_CHANGED,
            fit_columns_on_grid_load=False,
            height=300,  # Aumentada altura para melhor visualização
            # width removido para usar largura responsiva
            allow_unsafe_jscode=True,
            key=f"grid_{rota}",
            data_return_mode="AS_INPUT",
