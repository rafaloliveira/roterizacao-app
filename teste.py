...
def preparar_grid_detalhamento(df_fechadas: pd.DataFrame) -> pd.DataFrame:
    ...
    grp['Total_Contratado_Por_Rota'] = grp.groupby(col_rota)['Custo_Total_Contratado_Justificativa'].transform('sum')

    # Calcula a porcentagem do custo da justificativa em relação ao custo total da rota
    # Trata divisão por zero, caso o custo total da rota seja 0
    grp['% Custo Justificativa na Rota'] = (grp['Custo_Total_Contratado_Justificativa'] / grp['Total_Contratado_Por_Rota'] * 100).fillna(0)
    ...
    grp = grp[['Rota', 'Justificativas', 'Qtd de Cargas', 'Qtd de Entregas', '% Custo Justificativa na Rota']]
                    ...
                    if st.button(f"💾 Salvar Informações", key=salvar_key, disabled=save_button_disabled):
                        ...
                        try:
                            with st.spinner("Salvando dados da carga..."):
                                ...
                                updated_total_frete_base = df_carga_raw["Valor do Frete"].sum() # Use o valor base da carga
                                updated_total_frete_carga = updated_total_frete_base + valor_adicional_frete # Use o input do adicional
                        ...
                        # --- FIM DO AJUSTE ---
                            ...
                            try:
                                with st.spinner(" Enviando entregas para aprovação de custos..."):
                                    ...
                                    current_total_frete_base_send = df_carga_raw["Valor do Frete"].sum()
...
def pagina_aprovacao_custos():
    ...
    try: # <--- TRY PRINCIPAL DA FUNÇÃO
                ...
                try:
                    with st.spinner("🔄 Aprovando todas as entregas..."):
                        ...
                        cargas_agrupadas = df_aprovar_todas.groupby('numero_carga').agg(
                            ...
                            total_frete_base=('Valor do Frete', 'sum'),
        ...
        for carga in cargas_unicas:
            ...
            total_frete_carga = df_carga["Valor do Frete"].sum() # <-- Esta variável será usada nas chamadas à _aprovar_carga_custos
...
def pagina_cargas_fechadas():
    ...
    colunas_exibir = [
        ...
        "fechador_carga_login", "justificativa_aprovacao_custo",
        "custo_percentual_frete", "rentabilidade_percentual", "situacao_custo_regional",
        "pdf_downloaded_at", "csv_downloaded_at","carga_transf_filial"
    ]
    ...
    for carga in cargas_unicas_paginated:
        ...
        if df_carga.empty:
            # DEBUGs: Verificação de df_carga IMEDIATAMENTE após a criação
        ...
        total_frete_limpo = df_carga["Valor do Frete"].sum() + info_valor_adicional_raw
