...
def _aprovar_carga_custos(selecionadas_para_aprovar, df_carga_original, carga_num, justificativa_necessaria_param,selected_justification_text, new_observacao_aprovacao,  just_key_session_aprov_var, confirm_justify_key_session_aprov_var, just_key_session_rejeicao_var, confirm_justify_key_session_rejeicao_var, grid_key_var):
    ...
    if 'Previsao de Entrega' in df_aprovar.columns:
        ...
        # Conversão para Decimal para cálculos precisos
        getcontext().prec = 28 # Garante precisão para cálculos decimais
        d_valor_contratacao_carga = Decimal(str(valor_contratacao_limpo))
        d_total_frete_carga = Decimal(str(total_frete_limpo))
        ...
        if not d_total_frete_carga.is_zero() and d_total_frete_carga.is_finite() and not d_total_frete_carga.is_nan() and \
            ...
            if 'Regiao' in df_aprovar_full_columns.columns and not df_aprovar_full_columns['Regiao'].empty:
                regions_to_consider = df_aprovar_full_columns['Regiao'][df_aprovar_full_columns['Regiao'] != 'NÃO DEFINIDA']
                if not regions_to_consider.empty:
                    dominant_region = regions_to_consider.value_counts().idxmax()
                elif not df_aprovar_full_columns['Regiao'].empty:
                    dominant_region = df_aprovar_full_columns['Regiao'].iloc[0]
            ...
            if custo_receita_ratio <= max_cost_allowed:
                situacao_custo_regional_calc = f"Dentro do Limite ({(max_cost_allowed * 100).quantize(Decimal('1'), rounding=ROUND_HALF_UP)}%)"
            else:
                situacao_custo_regional_calc = f"Acima do Limite Região ({(max_cost_allowed * 100).quantize(Decimal('1'), rounding=ROUND_HALF_UP)}%)"
        elif valor_contratacao_limpo > 0:
        ...
        # Adiciona os novos campos calculados ao DataFrame df_aprovar
        # Converte Decimal para float para ser compatível com o tipo REAL no Supabase.
...
def gerar_pdf_carga(df_entregas, carga, rota, motorista, placa, veiculo, valor_frete, valor_contratacao, valor_sugerido_contratacao=None, for_print=False, data_hora_gerada=None):
    ...
    doc = SimpleDocTemplate(
        ...
        rightMargin=inch / 2,
        leftMargin=inch / 10, # <--- Alterado para inch / 4 (0.25 polegadas)
        topMargin=inch / 2,
        bottomMargin=inch / 2,
    ...
    requested_order_keys = [
        ...
        "Entrega Programada", "Peso Calculado em Kg", "Peso Real em Kg", "Cubagem em m³","Particularidade",
        "Serie_Numero_CTRC"
    ]
...
def get_all_motoristas_options():
    df_motoristas_data = load_motoristas_data() # Esta já estaria cacheada
                        ...
                        try:
                            with st.spinner(f"Gerando PDF COMPLETO para a carga {carga}... Por favor, aguarde..."):
                                ...
                                buffer_pdf = gerar_pdf_carga(
                                    ...
                                    data_hora_gerada=info_data_hora_gerada
                                )
            ...
            if st.button("✅ Aprovar Todas as Entregas da Página"):
                try:
                    with st.spinner("🔄 Aprovando todas as entregas..."):
                        ...
                        # Calcular esses valores a nível da carga (todos os itens na carga têm os mesmos valores de contratação)
                        valor_contratacao_carga_bruto = df_aprovar_todas["valor_contratacao"].iloc[0] if "valor_contratacao" in df_aprovar_todas.columns and not df_aprovar_todas.empty else 0.0
                        valor_adicional_frete_carga_bruto = df_aprovar_todas["valor_adicional_frete"].iloc[0] if "valor_adicional_frete" in df_aprovar_todas.columns and not df_aprovar_todas.empty else 0.0
                        
                        total_frete_base_bruto = df_aprovar_todas["Valor do Frete"].sum() if "Valor do Frete" in df_aprovar_todas.columns else 0.0
                        total_frete_carga_bruto = total_frete_base_bruto + valor_adicional_frete_carga_bruto
                        ...
                        if not d_total_frete_carga.is_zero() and d_total_frete_carga.is_finite() and not d_total_frete_carga.is_nan() and \
                           d_valor_contratacao_carga.is_finite() and not d_valor_contratacao_carga.is_nan():
                            ...
                            dominant_region = 'NÃO DEFINIDA'
                            if 'Regiao' in df_aprovar_todas.columns and not df_aprovar_todas['Regiao'].empty:
                                regions_to_consider = df_aprovar_todas['Regiao'][df_aprovar_todas['Regiao'] != 'NÃO DEFINIDA']
                                if not regions_to_consider.empty:
                                    dominant_region = regions_to_consider.value_counts().idxmax()
                                elif not df_aprovar_todas['Regiao'].empty:
                                    dominant_region = df_aprovar_todas['Regiao'].iloc[0]
    ...
    try:
        ...
        for carga in cargas_unicas:
            ...
            with col1_badges:
                ...
                all_badges_html_list.append(badge(f'Custo: {percentual_custo_texto}', background_color=cor_badge_custo, text_color=cor_texto_custo))
                all_badges_html_list.append(badge(f'Rentabilidade: {rentabilidade_percentual_db:.2f}%', background_color='#333333', text_color='white')) # Use _db aqui
                all_badges_html_list.append(badge(f'Situação Custo: {situacao_custo_regional_db}', background_color=cor_situacao, text_color='white')) # Use _db aqui
...
def pagina_cargas_fechadas():
        ...
        with st.spinner("Carregando dados para cargas encerradas..."):
            if should_reload_from_supabase:
                ...
                string_cols_to_clean = ['Regiao', 'motorista', 'placa', 'veiculo', 'situacao',
                                        ...
                                        'justificativa_aprovacao_custo', 'situacao_custo_regional']
    ...
    for carga in cargas_unicas_paginated:
        ...
        with col1_badges:
            ...
            all_badges_html_list.append(badge(f'Custo % Frete: {custo_percentual_frete_db:.2f}%', background_color=cor_badge_custo, text_color=cor_texto_custo))
